...
12. **NEVER change `unidad_compra` or `unidad_uso` when updating a price.** Only touch `precio_compra`/`cantidad_compra`/`proveedor`/`precios_proveedor`/`historial_precios`. Changing the unit of measure during a price update has broken cost calculations across sessions before — if the unit genuinely needs to change, that's a separate, explicit action requiring its own confirmation (see Safety section on `unidad_uso` changes), never a side effect of a price refresh
