...
import { BAR_SECTIONS, BAR_LABELS, getBarGroupForRecipe } from "@/lib/menu-data";
        ...
        <div className="space-y-8">
          {(() => {
            ...
            .flatMap((cat) => {
              ...
              if (cat.nombre === "Bebida") {
                return BAR_SECTIONS.map((group) => {
                  const items = catRecetas.filter((r) => getBarGroupForRecipe(r.nombre) === group);
                  if (items.length === 0) return null;
                  return (
                    <section key={`bebida-${group}`}>
                      <h2 className="text-lg font-bold mb-3">{BAR_LABELS[group]}</h2>
