  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <input required value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Categoría</label>
              <select value={form.categoria_id} onChange={(e) => setForm({ ...form, categoria_id: parseInt(e.target.value) })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
                {categorias.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Porciones por lote</label>
              <input type="number" min="1" value={form.porciones_por_lote} onChange={(e) => setForm({ ...form, porciones_por_lote: parseInt(e.target.value) || 1 })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Precio venta (CHF) — opcional</label>
              <input type="number" step="any" value={form.precio_venta ?? ""} onChange={(e) => setForm({ ...form, precio_venta: e.target.value ? parseFloat(e.target.value) : null })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm" />
            </div>
            ...
            {form.es_subreceta && (
              <div>
                <label className="block text-xs text-slate-500 mb-1">Unidad de rendimiento</label>
                <select value={form.unidad_rendimiento ?? ""} onChange={(e) => setForm({ ...form, unidad_rendimiento: e.target.value || null })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm">
          ...
          <div>
            ...
            <textarea value={form.notas} onChange={(e) => setForm({ ...form, notas: e.target.value })} className="w-full border border-slate-300 rounded px-3 py-2 text-sm" rows={2} />
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => addLinea("ingrediente")} className="text-sm text-blue-600 hover:text-blue-800">+ Ingrediente</button>
              {subrecetas.length > 0 && (
            ...
            <div className="space-y-2">
              {form.lineas.map((linea, i) => (
                <div key={i} className="flex flex-wrap items-center gap-2 p-2 bg-slate-50 rounded">
                  ...
                  <input type="number" step="any" value={linea.cantidad} onChange={(e) => updateLinea(i, "cantidad", parseFloat(e.target.value) || 0)} className="w-20 border border-slate-300 rounded px-2 py-1 text-sm" placeholder="Cant." />
        ...
        <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">Crear Receta</button>
      </form>
    </div>
  );
}
